...
import com.baidu.shop.server.GoodsService;
...
@RestController
@Slf4j
public class GoodsServiceImpl extends BaseApiServer implements GoodsService {
    @Resource
    ...
    @Override
    public Result<PageInfo<SpuEntity>> getSpuInfo(SpuDTO supDTO) {
        ...
        if(!ObjectUtils.isEmpty(supDTO.getSort()) && !ObjectUtils.isEmpty(supDTO.getOrder()))
            PageHelper.orderBy(supDTO.getOrder());
